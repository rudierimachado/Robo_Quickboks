...
# --- Funções do processo ---
def verificar_campo_wm():
    """
    Verifica se o campo 'Corsan - Wal-Mart.PNG' está preenchido com 'Corsan - Wal-Mart'.
    Retorna a mensagem 'job selecionado' se estiver preenchido.
    """
    try:
        logger.info("Verificando o campo 'Corsan - Wal-Mart.PNG'...")
        location = wait_for_image_cv2("Corsan - Wal-Mart.PNG", timeout=10)
        logger.info("Campo 'Corsan - Wal-Mart.PNG' detectado. Verificando preenchimento...")
        logger.info("job selecionado")
    except TimeoutError:
        logger.info("Campo 'Corsan - Wal-Mart.PNG' não detectado ou preenchido. Nenhuma ação necessária.")
...
def numero_invoice():
    ...
    insere o número INVmYY-WM (onde m é o mês sem zero à esquerda e YY é o ano atual),
    ...
    try:
        ...
        numero_invoice = f"INV{mes_passado.month}{mes_passado.strftime('%y')}-WM".upper()
...
def processar_planilha():
    """
    Verifica a planilha e realiza as operações de cadastrar o pedido, iniciando da próxima linha não marcada como 'OK' na coluna J.
    """
    try:
        ...
        sheet = wb.active 
        # Encontra a primeira linha não marcada como "OK" na coluna J
        linha_atual = 4  # Início padrão
        while sheet[f"J{linha_atual}"].value == "OK":
            linha_atual += 1
        ...
        while True:
            ...
            valor_h = sheet[f"H{linha_atual}"].value
            valor_i = sheet[f"I{linha_atual}"].value
            ...
            if codigo is None:  # Fim dos dados
                ...
                try:
                    ...
                    # Chamar o próximo script 7-wf.py
                    logger.info("Executando o script '7-wf.py'...")
                    subprocess.run(["python", "7-wf.py"], check=True)
                    logger.info("Script '7-wf.py' executado com sucesso.")
                    
                except Exception as e:
                    logger.error(f"Erro ao processar 'save_new.PNG' ou executar '7-wf.py': {e}")
                break


            if valor_h == 0 or valor_i == 0:  # Pular se valores forem zero
                logger.info(f"Linha {linha_atual} possui valor_h ou valor_i igual a 0. Pulando...")
                linha_atual += 1
                continue
            ...
            try:
                ...
                # Inserir valor de H e avançar "valor de H e a quantidade"
                logger.info(f"Inserindo valor de H: {valor_h}")
                pyautogui.typewrite(str(valor_h))
                pyautogui.press("tab")
                ...
                # Colocar o valor de I que é o US$
                valor_i_formatado = f"{valor_i:.2f}".replace('.', ',')
                logger.info(f"Inserindo valor de I: {valor_i_formatado}")
                pyautogui.typewrite(valor_i_formatado)
                pyautogui.press("tab")

                # Esperar para ir para o próximo pedido
                time.sleep(0.5)

                # Marcar como OK na coluna J
                sheet[f"J{linha_atual}"] = "OK"
...
def clicar_job_WM():
    ...
    try:
        ...
        logger.info("Colando o texto 'Corsan - Wal-Mart'...")
        pyperclip.copy("Corsan - Wal-Mart")  # Copia o texto para a área de transferência
...
def main():
    try:
        ...
        # Função desabilitada até poder salvar e continuar
...
def chamar_proximo_script():
    # Chama o script 7-wf.py após o término da execução do script principal.
    try:
        logger.info("Chamando o script '7-wf.py'...")
        subprocess.run(["python", "7-wf.py"], check=True)
        logger.info("Script '7-wf.py' executado com sucesso.")
    except subprocess.CalledProcessError as e:
        logger.error(f"Erro ao executar o script '7-wf.py': {e}")
    except FileNotFoundError:
        logger.error("O arquivo '7-wf.py' não foi encontrado. Verifique o caminho.")
