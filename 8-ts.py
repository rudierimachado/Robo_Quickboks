...
# --- Funções do processo ---
def verificar_campo_ts():
    """
    Verifica se o campo 'Corsan - Tractor Supply.PNG' está preenchido com 'Corsan - Tractor Supply'.
    Retorna a mensagem 'job selecionado' se estiver preenchido.
    """
    try:
        logger.info("Verificando o campo 'Corsan - Tractor Supply.PNG'...")
        location = wait_for_image_cv2("Corsan - Tractor Supply.PNG", timeout=10)
        logger.info("Campo 'Corsan - Tractor Supply.PNG' detectado. Verificando preenchimento...")
        logger.info("job selecionado")
    except TimeoutError:
        logger.info("Campo 'Corsan - Tractor Supply.PNG' não detectado ou preenchido. Nenhuma ação necessária.")
...
def clicar_job_ts():
    ...
    try:
        ...
        logger.info("Colando o texto 'Corsan - Tractor Supply'...")
        pyperclip.copy("Corsan - Tractor Supply")  # Copia o texto para a área de transferência
...
def numero_invoice():
    ...
    insere o número INVmYY-TS (onde m é o mês sem zero à esquerda e YY é o ano atual),
    ...
    try:
        ...
        numero_invoice = f"INV{mes_passado.month}{mes_passado.strftime('%y')}-TS".upper()
...
def processar_planilha():
    ...
    try:
        ...
        while sheet[f"P{linha_atual}"].value == "OK":
            linha_atual += 1
        ...
        while True:
            ...
            valor_n = sheet[f"N{linha_atual}"].value
            valor_o = sheet[f"O{linha_atual}"].value
            ...
            if codigo is None:  # Fim dos dados
                ...
                try:
                    ...
                    logger.info("Executando o script '9-rd.py'...")
                    subprocess.run(["python", "9-rd.py"], check=True)
                    logger.info("Script '9-rd.py' executado com sucesso.")
                    
                except Exception as e:
                    logger.error(f"Erro ao processar 'save_new.PNG' ou executar '9-rd.py': {e}")
                break

            if valor_n == 0 or valor_o == 0:  # Pular se valores forem zero
                logger.info(f"Linha {linha_atual} possui valor_n ou valor_o igual a 0. Pulando...")
                linha_atual += 1
                continue
            ...
            try:
                ...
                # Inserir a quantidade (valor_n) e avançar
                logger.info(f"Inserindo valor de N: {valor_n}")
                pyautogui.typewrite(str(valor_n))
                pyautogui.press("tab")
                ...
                # Colocar o valor de O que é o US$
                valor_o_formatado = f"{valor_o:.2f}".replace('.', ',')
                logger.info(f"Inserindo valor de O: {valor_o_formatado}")
                pyautogui.typewrite(valor_o_formatado)
                pyautogui.press("tab")
...
def chamar_proximo_script():
      ...
      #Chama o script 9-rd.py após o término da execução do script principal.
   
    try:
        logger.info("Chamando o script '9-rd.py'...")
        subprocess.run(["python", "9-rd.py"], check=True)
        logger.info("Script '9-rd.py' executado com sucesso.")
    except subprocess.CalledProcessError as e:
        logger.error(f"Erro ao executar o script '9-rd.py': {e}")
    except FileNotFoundError:
        logger.error("O arquivo '9-rd.py' não foi encontrado. Verifique o caminho.")
