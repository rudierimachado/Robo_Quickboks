...
# --- Funções do processo ---
def verificar_campo_wf():
    """
    Verifica se o campo 'Corsan_way fair.PNG' está preenchido com 'Corsan - Way Fair'.
    Retorna a mensagem 'job selecionado' se estiver preenchido.
    """
    try:
        logger.info("Verificando o campo 'Corsan_way fair.PNG'...")
        location = wait_for_image_cv2("Corsan_way fair.PNG", timeout=10)
        logger.info("Campo 'Corsan_way fair.PNG' detectado. Verificando preenchimento...")
        logger.info("job selecionado")
    except TimeoutError:
        logger.info("Campo 'Corsan_way fair.PNG' não detectado ou preenchido. Nenhuma ação necessária.")
...
def clicar_job_wf():
    ...
    try:
        ...
        logger.info("Colando o texto 'Corsan - Way Fair'...")
        pyperclip.copy("Corsan - Way Fair")  # Copia o texto para a área de transferência
...
def numero_invoice():
    ...
    insere o número INVmYY-WF (onde m é o mês sem zero à esquerda e YY é o ano atual),
    ...
    try:
        ...
        numero_invoice = f"INV{mes_passado.month}{mes_passado.strftime('%y')}-WF".upper()
...
def processar_planilha():
    ...
    try:
        ...
        while sheet[f"m{linha_atual}"].value == "OK":
            linha_atual += 1
        ...
        while True:
            ...
            valor_k = sheet[f"K{linha_atual}"].value
            valor_l = sheet[f"L{linha_atual}"].value
            ...
            if codigo is None:  # Fim dos dados
                ...
                try:
                    ...
                    logger.info("Executando o script '8-ts.py'...")
                    subprocess.run(["python", "8-ts.py"], check=True)
                    logger.info("Script '8-ts.py' executado com sucesso.")
                    
                except Exception as e:
                    logger.error(f"Erro ao processar 'save_new.PNG' ou executar '8-ts.py': {e}")
                break

            if valor_k == 0 or valor_l == 0:  # Pular se valores forem zero
                logger.info(f"Linha {linha_atual} possui valor_k ou valor_l igual a 0. Pulando...")
                linha_atual += 1
                continue
            ...
            try:
                ...
                # Inserir a quantidade (valor_k) e avançar
                logger.info(f"Inserindo valor de K: {valor_k}")
                pyautogui.typewrite(str(valor_k))
                pyautogui.press("tab")
                ...
                # Colocar o valor de L que é o US$
                valor_l_formatado = f"{valor_l:.2f}".replace('.', ',')
                logger.info(f"Inserindo valor de L: {valor_l_formatado}")
                pyautogui.typewrite(valor_l_formatado)
                pyautogui.press("tab")
...
def chamar_proximo_script():
      ...
      #Chama o script 8-ts.py após o término da execução do script principal.
   
    try:
        logger.info("Chamando o script '8-ts.py'...")
        subprocess.run(["python", "8-ts.py"], check=True)
        logger.info("Script '8-ts.py' executado com sucesso.")
    except subprocess.CalledProcessError as e:
        logger.error(f"Erro ao executar o script '8-ts.py': {e}")
    except FileNotFoundError:
        logger.error("O arquivo '8-ts.py' não foi encontrado. Verifique o caminho.")
